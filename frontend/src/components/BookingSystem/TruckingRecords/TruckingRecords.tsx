...
class TruckingRecords extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.overview !== this.props.overview) {
      this.props.fetchTruckingRecords(0,10, this.overViewType[this.props.overview.overviewType - 1]
      );
    }
  }
  ...
  render() {
    const { loading, error, total, visibleColumns = [] } = this.props;
    ...
    const allFeeNames = Array.from(
      new Set(sortedRecords.flatMap((rec) => rec.fees || []).map((f) => f.fee?.fee_name))
    );
          ...
          <table className="w-full bg-white shadow rounded text-s text-center">
            ...
            <tbody>
              {sortedRecords.length === 0 ? (
                <tr>
                  <td colSpan={6 + allFeeNames.length} className="p-4 text-sm text-gray-500">
                    No records found.
                  </td>
                </tr>
              ) : (
                sortedRecords.map((rec) => {
                  ...
                  return (
                    <tr key={rec.id} className="border-t hover:bg-gray-50">
                      {visibleColumns.includes('record_date') && <td className="p-1">{rec.record_date}</td>}
                      {visibleColumns.includes('record_time') && <td className="p-1">{rec.record_time}</td>}
                      {visibleColumns.includes('destination') && <td className="p-1">{rec.destination}</td>}
                      {visibleColumns.includes('plate_number') && <td className="p-1">{rec.plate_number}</td>}
                      {visibleColumns.includes('vessel.vessel_name') && <td className="p-1">{rec.vessel?.vessel_name}</td>}
                      {visibleColumns.includes('company.company_name') && <td className="p-1">{rec.company?.company_name}</td>}
                      {visibleColumns.includes('creator.name') && <td className="p-1">{rec.creator?.name}</td>}
